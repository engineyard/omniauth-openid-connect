...
class OmniAuth::Strategies::OpenIDConnect
  ...
  def request_phase
    if options.issuer.blank?
      options.issuer = issuer
    end

    options.discovery && discover!
  ...
  def callback_phase
    ...
    elsif request.params['state'] != stored_state && !(request.params['state'].blank? && stored_state.blank?)
  ...
  def authorize_uri
    ...
    opts = {
      ...
      :scope         => options.scope,
      :state         => new_state,
      :nonce         => options.send_nonce.presence && new_nonce,
    }
    client.authorization_uri(opts.reject{|k,v| v.nil?})
  end

  def public_key
    options.discovery ? config.public_keys.first : key_or_secret
  end
  ...
  def user_info
    @user_info ||= if client_options.userinfo_endpoint
                     access_token.userinfo!
                   elsif access_token.id_token
                     ::OpenIDConnect::ResponseObject::UserInfo.new(access_token.id_token.raw_attributes)
                   else
                     raise "can't fetch user info. no id_token or userinfo_endpoint available"
                   end
  end
  ...
  def new_state
    session['omniauth.state'] = (options.state.respond_to?(:call) ? options.state.call : SecureRandom.hex(16))
  end
